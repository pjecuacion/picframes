...
class SourceCard(ctk.CTkFrame):
    ...
    def _build(
        ...
        ctk.CTkLabel(
            drop_zone,
            text="Drag files or a folder here",  # TODO: mention your file type(s)
        ...
        ctk.CTkLabel(
            drop_zone,
            # TODO: update to describe your supported input format(s)
            text="Drop supported files for file mode or a single folder for bulk mode.",
            text_color=theme.TEXT_MUTED,
            font=theme.font(12),
        ).grid(row=1, column=0, padx=16, pady=(0, 14))

    def pick(self, mode: str) -> tuple[list[Path], Path | None]:
        """Open the appropriate file/folder dialog and return (files, folder)."""
        from tkinter import filedialog
        if mode == "files":
            paths = filedialog.askopenfilenames(
                title="Choose files",
                # TODO: replace with your app's accepted input formats, e.g.:
    ...
    def sync(self, mode: str, files: list[Path], folder: Path | None) -> None:
        """Update summary label and pick button text to reflect current state."""
        if mode == "files":
            self.pick_button.configure(text="Select Files")
            count = len(files)
            if count == 0:
                self.source_summary.configure(text="No files selected yet.")
            else:
                preview = "\n".join(str(p) for p in files[:3])
                suffix = "" if count <= 3 else f"\n...and {count - 3} more"
                self.source_summary.configure(text=f"{count} files selected\n{preview}{suffix}")
        else:
