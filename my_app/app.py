...
APP_TITLE = "My App"  # TODO: rename to your app title


class MyApp(ctk.CTk, TkinterDnD.DnDWrapper):
    ...
    def _on_drop(self, event):
        ...
        if files:
            ...
            pc.status_label.configure(text=f"Loaded {len(files)} dropped file(s).")
        elif len(folders) == 1:
            ...
            messagebox.showinfo(APP_TITLE, "Drop supported files or a single folder.")
        self._sync_view()
        return COPY
    ...
    def _run_job(self) -> None:
        ...
        if not sources:
            messagebox.showinfo(APP_TITLE, "Select at least one file or a folder containing supported files.")
            return
        ...
        if not jobs:
            messagebox.showinfo(APP_TITLE, "No supported files were found.")
            return
    ...
    def _finish_job(self, results, output_dir: Path) -> None:
        ...
        self._sync_view()
        if failed == 0:
            messagebox.showinfo(APP_TITLE, f"Finished processing {processed} file(s) to {output_dir}")
        else:
